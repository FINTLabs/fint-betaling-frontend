...
const RecipientSearch = () => {
  ...
  function matchedSuggestion(suggestion, input) {
    if (recipientType === GROUP) {
      return suggestion.navn.slice(0, input.length)
        .toLowerCase() === input;
    }
    return (
      (suggestion.fulltNavn && suggestion.fulltNavn.slice(0, input.length)
        .toLowerCase() === input)
      || (suggestion.navn.fornavn && suggestion.navn.fornavn.slice(0, input.length)
        .toLowerCase() === input)
      || (suggestion.navn.mellomnavn && suggestion.navn.mellomnavn.slice(0, input.length)
        .toLowerCase() === input)
      || (suggestion.navn.etternavn && suggestion.navn.etternavn.slice(0, input.length)
        .toLowerCase() === input)
    );
  }
    ...
    <Box className={classes.root}>
      <Paper className={classes.container}>
        <Autosuggest
          ...
          onSuggestionsöFetchRequested={handleSuggestionsFetchRequested}
