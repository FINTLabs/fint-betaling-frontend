    ...
    return inputLength < 0
        ...
        : suggestions.filter((suggestion) => {
            ...
            if (searchBy === ORDER_NUMBER) {
                matched = suggestion.orderNumber.toString()
                    .slice(0, input.length)
                    .toLowerCase() === input && (claimStatus === filter || filter === FILTER_ALL);
                if (matched) {
                ...
                matched = suggestion.customer.name.toLowerCase()
                    .includes(input.toLowerCase()) && (claimStatus === filter || filter === FILTER_ALL);
                if (matched) {
                    countToStartOfActivePage += 1;
                    if (countToStartOfActivePage >= keepSuggestionsFromCount) {
                        keep = count < 10
